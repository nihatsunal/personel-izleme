...
/*
 * $Revision: 5369 $
 * $Id: UserSkillListFactory.class.php 5369 2011-10-21 19:37:24Z ipso $
...
class UserSkillListFactory extends UserSkillFactory implements IteratorAggregate {
    ...
    function  getAPISearchByCompanyIdAndArrayCriteria( $company_id, $filter_data, $limit = NULL, $page = NULL, $where = NULL, $order = NULL ) {
		...
		if ( isset($filter_data['proficiency']) AND trim($filter_data['proficiency']) != '' AND !isset($filter_data['proficiency_id']) ) {
		...
		$query .= ( isset($filter_data['experience']) ) ? $this->getWhereClauseSQL( 'a.experience', $filter_data['experience'], 'text', $ph ) : NULL;
        ...
        if ( isset($filter_data['skill_expiry_start_date']) AND trim($filter_data['skill_expiry_start_date']) != '' ) {
			$ph[] = $filter_data['skill_expiry_start_date'];
			$query  .=	' AND a.expiry_date >= ?';
		}
		if ( isset($filter_data['skill_expiry_end_date']) AND trim($filter_data['skill_expiry_end_date']) != '' ) {
			$ph[] = $filter_data['skill_expiry_end_date'];
			$query  .=	' AND a.expiry_date <= ?';
		}
