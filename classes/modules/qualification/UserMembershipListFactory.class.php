...
/*
 * $Revision: 5369 $
 * $Id: UserMembershipListFactory.class.php 5369 2011-10-21 19:37:24Z ipso $
...
class UserMembershipListFactory extends UserMembershipFactory implements IteratorAggregate {
    ...
    function  getAPISearchByCompanyIdAndArrayCriteria( $company_id, $filter_data, $limit = NULL, $page = NULL, $where = NULL, $order = NULL ) {
		...
		if ( isset($filter_data['ownership']) AND trim($filter_data['ownership']) != '' AND !isset($filter_data['ownership_id']) ) {
        ...
        if ( isset($filter_data['membership_renewal_start_date']) AND trim($filter_data['membership_renewal_start_date']) != '' AND is_numeric($filter_data['membership_renewal_start_date']) ) {
			$ph[] = $filter_data['membership_renewal_start_date'];
			$query  .=	' AND a.renewal_date >= ?';
		}
		if ( isset($filter_data['membership_renewal_end_date']) AND trim($filter_data['membership_renewal_end_date']) != '' ) {
			$ph[] = $filter_data['membership_renewal_end_date'];
			$query  .=	' AND a.renewal_date <= ?';
		}
