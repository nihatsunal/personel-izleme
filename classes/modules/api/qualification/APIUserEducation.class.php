...
/*
 * $Revision: 2196 $
 * $Id: APIUserEducation.class.php 2196 2008-10-14 16:08:54Z ipso $
...
class APIUserEducation extends APIFactory {
	...
	function getCommonUserEducationData( $data ) {
		return Misc::arrayIntersectByRow( $this->getUserEducation( $data, TRUE ) );
	}
	 ...
	 * @return array
	 */
	function setUserEducation( $data, $validate_only = FALSE ) {
		...
		if ( is_array($data) ) {
			...
			foreach( $data as $key => $row ) {
				...
				if ( isset($row['id']) AND $row['id'] > 0 ) {
					...
					$primary_validator->isTrue( 'permission', $this->getPermissionObject()->Check('user_education','add'), TTi18n::gettext('Add permission denied') );
				}
	...
	function copyUserEducation( $data ) {
		...
		$src_rows = $this->getUserEducation( array('filter_data' => array('id' => $data) ), TRUE );
