...
class APIUserLicense extends APIFactory {
	...
	function getCommonUserLicenseData( $data ) {
		return Misc::arrayIntersectByRow( $this->getUserLicense( $data, TRUE ) );
	}
	 ...
	 * @return array
	 */
	function setUserLicense( $data, $validate_only = FALSE ) {
		...
		if ( is_array($data) ) {
			...
			foreach( $data as $key => $row ) {
				...
				if ( isset($row['id']) AND $row['id'] > 0 ) {
					...
					$primary_validator->isTrue( 'permission', $this->getPermissionObject()->Check('user_license','add'), TTi18n::gettext('Add permission denied') );
				}
	...
	function copyUserLicense( $data ) {
		...
		$src_rows = $this->getUserLicense( array('filter_data' => array('id' => $data) ), TRUE );
