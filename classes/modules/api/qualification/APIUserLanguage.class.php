...
class APIUserLanguage extends APIFactory {
	...
	function getCommonUserLanguageData( $data ) {
		return Misc::arrayIntersectByRow( $this->getUserLanguage( $data, TRUE ) );
	}
		...
		if ( is_array($data) ) {
			...
			foreach( $data as $key => $row ) {
				...
				if ( isset($row['id']) AND $row['id'] > 0 ) {
					...
					$primary_validator->isTrue( 'permission', $this->getPermissionObject()->Check('user_language','add'), TTi18n::gettext('Add permission denied') );
				}
	...
	function copyUserLanguage( $data ) {
		...
		$src_rows = $this->getUserLanguage( array('filter_data' => array('id' => $data) ), TRUE );
