...
class APICurrency extends APIFactory {
	...
	function getCommonCurrencyData( $data ) {
		return Misc::arrayIntersectByRow( $this->getCurrency( $data, TRUE ) );
	}
	 ...
	 * @return array
	 */
	function setCurrency( $data, $validate_only = FALSE ) {
	...
	function copyCurrency( $data ) {
		...
		$src_rows = $this->getCurrency( array('filter_data' => array('id' => $data) ), TRUE );
