...
/*
 * $Revision: 2196 $
 * $Id: APIHoliday.class.php 2196 2008-10-14 16:08:54Z ipso $
...
class APIHoliday extends APIFactory {
	...
	function getHoliday( $data = NULL, $disable_paging = FALSE ) {
		...
		if ( !( $this->getPermissionObject()->Check('holiday_policy','enabled') OR $this->getPermissionObject()->Check('schedule','enabled') )
	 ...
	 * @return array
	 */
	function getCommonHolidayData( $data ) {
		return Misc::arrayIntersectByRow( $this->getHoliday( $data, TRUE ) );
	}

	/**
	 * Validate holiday data for one or more holidayes.
	 * @param array $data holiday data
	 * @return array
	 */
	function validateHoliday( $data ) {
		return $this->setHoliday( $data, TRUE );
	}
	 ...
	 * @return array
	 */
	function setHoliday( $data, $validate_only = FALSE ) {
	...
	function deleteHoliday( $data ) {
		...
		Debug::Text('Received data for: '. count($data) .' Holidays', __FILE__, __LINE__, __METHOD__, 10);
		Debug::Arr($data, 'Data: ', __FILE__, __LINE__, __METHOD__, 10);
	...
	/**
	 * Copy one or more holidayes.
	 * @param array $data holiday IDs
	 * @return array
	 */
	function copyHoliday( $data ) {
		...
		$src_rows = $this->getHoliday( array('filter_data' => array('id' => $data) ), TRUE );
		if ( is_array( $src_rows ) AND count($src_rows) > 0 ) {
			...
			return $this->setHoliday( $src_rows ); //Save copied rows
		}
