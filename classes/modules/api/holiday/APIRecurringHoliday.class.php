...
/*
 * $Revision: 2196 $
 * $Id: APIRecurringHoliday.class.php 2196 2008-10-14 16:08:54Z ipso $
 ...
 * @package API\Holiday
 */
class APIRecurringHoliday extends APIFactory {
	protected $main_class = 'RecurringHolidayFactory';
	...
	/**
	 * Get default recurring holiday data for creating new recurring holidayes.
	 * @return array
	 */
	function getRecurringHolidayDefaultData() {
		$company_obj = $this->getCurrentCompanyObject();

		Debug::Text('Getting recurring holiday default data...', __FILE__, __LINE__, __METHOD__,10);
	...
	/**
	 * Get recurring holiday data for one or more recurring holidayes.
	 * @param array $data filter data
	 * @return array
	 */
	function getRecurringHoliday( $data = NULL, $disable_paging = FALSE ) {
		...
		$blf = TTnew( 'RecurringHolidayListFactory' );
	...
	/**
	 * Validate recurring holiday data for one or more recurring holidayes.
	 * @param array $data recurring holiday data
	 * @return array
	 */
	function validateRecurringHoliday( $data ) {
		return $this->setRecurringHoliday( $data, TRUE );
	}

	/**
	 * Set recurring holiday data for one or more recurring holidayes.
	 * @param array $data recurring holiday data
	 * @return array
	 */
	function setRecurringHoliday( $data, $validate_only = FALSE ) {
		...
		Debug::Text('Received data for: '. $total_records .' RecurringHolidays', __FILE__, __LINE__, __METHOD__, 10);
		...
		if ( is_array($data) ) {
			foreach( $data as $key => $row ) {
				...
				if ( isset($row['id']) AND $row['id'] > 0 ) {
					...
					//Get recurring holiday object, so we can only modify just changed data for specific records if needed.
	...
	/**
	 * Delete one or more recurring holidays.
	 * @param array $data recurring holiday data
	 * @return array
	 */
	function deleteRecurringHoliday( $data ) {
		if ( is_numeric($data) ) {
			$data = array($data);
		}
	...
	/**
	 * Copy one or more recurring holidayes.
	 * @param array $data recurring holiday IDs
	 * @return array
	 */
	function copyRecurringHoliday( $data ) {
		if ( is_numeric($data) ) {
			$data = array($data);
		}
		...
		if ( is_array( $src_rows ) AND count($src_rows) > 0 ) {
			...
			return $this->setRecurringHoliday( $src_rows ); //Save copied rows
		}
