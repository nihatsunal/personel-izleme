 ...
 * @package API\Policy
 */
class APIRoundIntervalPolicy extends APIFactory {
	protected $main_class = 'RoundIntervalPolicyFactory';
	...
	/**
	 * Get default round interval policy data for creating new round interval policyes.
	 * @return array
	 */
	...
	/**
	 * Get round interval policy data for one or more round interval policyes.
	...
	function getRoundIntervalPolicy( $data = NULL, $disable_paging = FALSE ) {
		...
		$data['filter_data']['permission_children_ids'] = $this->getPermissionObject()->getPermissionChildren( 'round_policy', 'view' );

		$blf = TTnew( 'RoundIntervalPolicyListFactory' );
	 ...
	 * @return array
	 */
	function getCommonRoundIntervalPolicyData( $data ) {
		return Misc::arrayIntersectByRow( $this->getRoundIntervalPolicy( $data, TRUE ) );
	}

	/**
	 * Validate round interval policy data for one or more round interval policyes.
	 * @param array $data round interval policy data
	 * @return array
	 */
	function validateRoundIntervalPolicy( $data ) {
		return $this->setRoundIntervalPolicy( $data, TRUE );
	}

	/**
	 * Set round interval policy data for one or more round interval policyes.
	 * @param array $data round interval policy data
	 * @return array
	 */
	function setRoundIntervalPolicy( $data, $validate_only = FALSE ) {
		...
		Debug::Text('Received data for: '. $total_records .' RoundIntervalPolicys', __FILE__, __LINE__, __METHOD__, 10);
		...
		if ( is_array($data) ) {
			foreach( $data as $key => $row ) {
				...
				if ( isset($row['id']) AND $row['id'] > 0 ) {
					...
					//Get round interval policy object, so we can only modify just changed data for specific records if needed.
	...
	/**
	 * Delete one or more round interval policys.
	 * @param array $data round interval policy data
	 * @return array
	 */
	function deleteRoundIntervalPolicy( $data ) {
		if ( is_numeric($data) ) {
			$data = array($data);
		}
	...
	/**
	 * Copy one or more round interval policyes.
	 * @param array $data round interval policy IDs
	 * @return array
	 */
	function copyRoundIntervalPolicy( $data ) {
		if ( is_numeric($data) ) {
			$data = array($data);
		}
		...
		$src_rows = $this->getRoundIntervalPolicy( array('filter_data' => array('id' => $data) ), TRUE );
		if ( is_array( $src_rows ) AND count($src_rows) > 0 ) {
			...
			return $this->setRoundIntervalPolicy( $src_rows ); //Save copied rows
		}
