...
class APIAccrualPolicyMilestone extends APIFactory {
	 ...
	 * @return array
	 */
	function setAccrualPolicyMilestone( $data, $validate_only = FALSE ) {
	 ...
	 */
/*
	function copyAccrualPolicyMilestone( $data ) {
		if ( is_numeric($data) ) {
			$data = array($data);
		}
		...
		if ( is_array( $src_rows ) AND count($src_rows) > 0 ) {
			...
			return $this->setAccrualPolicyMilestone( $src_rows ); //Save copied rows
		}
